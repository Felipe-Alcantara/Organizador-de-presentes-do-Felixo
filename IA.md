...
[2026-06-01] Seletor de grade (`#gridCols`): Auto / 2 a 8 colunas por linha via atributo `data-cols` no `.grid` (CSS `grid-template-columns:repeat(N,1fr)`). 'auto' remove o override e volta ao `auto-fill minmax(230px,1fr)`. Fallback responsivo: ≤1200px as opções 6-8 caem para 4; ≤900px tudo (3-8) cai para 2; ≤560px qualquer grade fixa vira 1 coluna.  
...
[2026-07-06] Alternador Grade/Lista (`#viewGrid`/`#viewList`, segmentado) inspirado no projeto Git-Hub-Repositories. Estado `viewMode` ('grid'|'list') declarado no topo (junto de `gifts`/`editingId`) — declarar `let` na seção de persistência causava TDZ ("Cannot access 'viewMode' before initialization") porque `loadPrefs()`/`applyViewMode()` são chamados na init antes daquela linha. No modo lista, `.grid.list-view` vira flex-column e `.gift-card` vira linha horizontal (miniatura 120px + título/preço/status + ações à direita); tags e notas ocultas para linhas uniformes; seletor de colunas escondido. Persistido em `felixo_gifts_prefs_v1` ao lado de `gridCols`.  
